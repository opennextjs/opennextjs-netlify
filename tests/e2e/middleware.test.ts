...
for (const { expectedRuntime, isNodeMiddleware, label, testWithSwitchableMiddlewareRuntime } of [
  ...
  test.describe(
    label,
    {
      tag: generateTestTags({ middleware: isNodeMiddleware ? 'node' : 'edge' }),
    },
    () => {
      test.describe(
        'With App Router',
        ...
        () => {
          ...
          test('does not run middleware again for rewrite target', async ({
            ...
            const rewritten = await page.goto(
              `${edgeOrNodeMiddleware.url}/test/rewrite-loop-detect`,
            )
          ...
          test('Supports CJS dependencies in Edge Middleware', async ({
            page,
            edgeOrNodeMiddleware,
          }) => {
            const res = await page.goto(`${edgeOrNodeMiddleware.url}/test/next`)
          ...
          test.describe('RSC cache poisoning', () => {
            ...
            test('Middleware redirect', async ({ page, edgeOrNodeMiddleware }) => {
              const prefetchResponsePromise = new Promise<Response>((resolve) => {
                page.on('response', (response) => {
                  if (
                    response.url().includes('/caching-redirect-target') &&
                    response.status() === 200
                  ) {
                    resolve(response)
                  }
            ...
            test.describe('Node.js Middleware specific', () => {
              ...
              test.describe('pnpm package manager', () => {
                test('node:crypto module', async ({ middlewareNodeRuntimeSpecificPnpm }) => {
                  const response = await fetch(
                    `${middlewareNodeRuntimeSpecificPnpm.url}/test/crypto`,
                  )
                  expect(response.status).toBe(200)
                  const body = await response.json()
                  expect(
        ...
        () => {
          test.describe('json data', () => {
            ...
            test.describe(
              'with 18n',
              {
                tag: generateTestTags({ i18n: true }),
              },
              () => {
                for (const testConfig of testConfigs) {
                  test.describe(testConfig.describeLabel, () => {
                    for (const { localeLabel, pageWithLinksPathname } of [
                      ...
                      test.describe(localeLabel, () => {
                        test('json data fetch', async ({ edgeOrNodeMiddlewareI18n, page }) => {
                          ...
                          expect(await pageResponse?.headerValue('x-runtime')).toEqual(
                            expectedRuntime,
                          )
          ...
          test.describe(
            'Middleware with i18n and excluded paths',
            {
              tag: generateTestTags({ i18n: true }),
            },
            () => {
